...
const FIRELILY_DEVCHAIN = {
  relay: 'Firelily',
  name: 'Firelily DevChain',
  rpc: 'wss://devchain.firelily.net/ws',
};

export const FIRELILY_TESTCHAIN = {
  relay: 'Firelily',
  name: 'Firelily TestChain',
  rpc: 'wss://testchain.firelily.net/ws',
};

export const TESTNETS = [
  ...[FIRELILY_DEVCHAIN, FIRELILY_TESTCHAIN].sort((a, b) => a.name.localeCompare(b.name)),
  LOCAL,
];

type Chain = typeof FIRELILY_DEVCHAIN;

export const MAINNETS: Chain[] = ([] as Chain[]).sort((a, b) => a.name.localeCompare(b.name));
